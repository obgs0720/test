...
function submitQuiz() {
  ...
  state.progress.seen = [...new Set(state.progress.seen)]; saveProgress(); $('quizView').hidden = true; $('resultView').hidden = false; $('scoreText').textContent = `${score} / ${state.quiz.length}`; $('scoreMessage').textContent = score === state.quiz.length ? '全部答對，太棒了！' : '看完錯題解析，再挑戰一次。'; $('reviewList').innerHTML = state.quiz.map((q, i) => { const chosen = state.answers[q.id]; const ok = chosen === q.answer; return `<article class="review-card"><div class="review-head"><div><div class="question-number">QUESTION ${String(i+1).padStart(2,'0')}</div><h3 class="review-title">${escapeHtml(q.question)}</h3></div><span class="badge ${ok?'correct':'wrong'}">${ok?'答對':'答錯'}</span></div>${ok ? '' : `<p class="answer-note">正確答案：<strong>${String.fromCharCode(65+q.answer)}. ${escapeHtml(q.options[q.answer])}</strong></p>`}</article>`; }).join(''); }
